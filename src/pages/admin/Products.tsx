    ...
    <AdminLayout>
      <div className="p-10">
        <div className="flex items-center justify-between mb-8">
          ...
          <Button onClick={openNew}><Plus size={16} className="mr-2" />Nouveau produit</Button>
        </div>
              ...
              <div className="col-span-2">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button type="button" onClick={() => setPickerOpen(true)}
                    ...
                    className="flex items-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-900 border border-zinc-200 hover:border-zinc-400 rounded-lg px-2.5 py-1 transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
                    <Image size={12} />
                    ...
                    return url ? (
                      <div key={i} className="relative group">
                        ...
                        <button type="button"
                          ...
                          className="absolute -top-2 -right-2 bg-white border border-zinc-200 rounded-full p-0.5 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-opacity shadow-sm">
                      ...
                      <label key={i} className="aspect-square border-2 border-dashed border-zinc-200 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-zinc-400 hover:bg-zinc-50 transition-colors">
                        ...
                        <span className="text-[10px] text-zinc-300">{uploading && i === form.images.length ? 'Upload…' : 'Ajouter'}</span>
        ...
        <div className="bg-white rounded-2xl border border-zinc-100 overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-zinc-50 border-b border-zinc-100">
              <tr>
                <th className="text-left px-6 py-3 font-medium text-zinc-500 w-12"></th>
                <th className="text-left px-6 py-3 font-medium text-zinc-500">Nom</th>
                <th className="text-left px-6 py-3 font-medium text-zinc-500">Prix</th>
                <th className="text-left px-6 py-3 font-medium text-zinc-500">Catégorie</th>
                <th className="text-left px-6 py-3 font-medium text-zinc-500">Statut</th>
            ...
            <tbody className="divide-y divide-zinc-50">
              {products.map(p => (
                <tr key={p.id} className="hover:bg-zinc-50/50">
                  ...
                  <td className="px-6 py-3 text-zinc-600">{p.price}</td>
                  <td className="px-6 py-3 text-zinc-500">{p.category}</td>
                  <td className="px-6 py-3"><StatusBadge status={p.status} /></td>
                  <td className="px-6 py-3 text-right">
                    <button onClick={() => openEdit(p)} className="text-zinc-400 hover:text-zinc-900 mr-3"><Pencil size={14} /></button>
                    <button onClick={() => del(p.id)} className="text-zinc-400 hover:text-red-500"><Trash2 size={14} /></button>
                  </td>
                </tr>
              ))}
              {products.length === 0 && (
                <tr><td colSpan={6} className="px-6 py-8 text-center text-zinc-400">Aucun produit</td></tr>
              )}
            </tbody>
          </table>
        </div>
